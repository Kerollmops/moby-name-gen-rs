...
pub fn random_name() -> String {
    random_name_from_rng(&mut thread_rng())
}

/// Generates a random name *using the given rng* from the list of adjectives and surnames in this package
/// formatted as "adjective-surname". For example 'focused-turing'.
pub fn random_name_from_rng<R: Rng>(rng: &mut R) -> String {
    let adjective = rng.choose(ADJECTIVES).unwrap();
    let surname = rng.choose(SURNAMES).unwrap();
    String::from(*adjective) + "-" + surname
}
